...
public class YoutubeReportService {
    ...
    @Transactional
    public void reportVideo(Long userId, String videoId, String reasonId, String secondaryReasonId,
            String googleAccessToken) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("사용자를 찾을 수 없습니다."));

        // 분석된 영상인지 확인
        Video video = videoRepository.findByYtVideoId(videoId)
                .orElseThrow(() -> new RuntimeException("분석되지 않은 영상은 신고할 수 없습니다."));
        ...
        try {
            ...
            // 2. 이력 저장 (성공)
            saveHistory(user, video, reasonId, ReportStatus.SUCCESS);

        } catch (Exception e) {
            log.error("유튜브 신고 실패: userId={}, videoId={}, error={}", user.getId(), videoId, e.getMessage());

            // 3. 이력 저장 (실패)
            saveHistory(user, video, reasonId, ReportStatus.FAIL);
    ...
    private void saveHistory(User user, Video video, String reasonId, ReportStatus status) {
