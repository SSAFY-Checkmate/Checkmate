...
public class AnalysisJobService {
	...
	public AnalysisJob createJob(String youtubeUrl) {
		...
		AnalysisJob job = AnalysisJob.requestedFull(normalized);
		analysisJobRepository.save(job);
		return job;
	}

	@Transactional
	public AnalysisJob createJobRange(String youtubeUrl, double startSeconds, double endSeconds) {
		String normalized = YoutubeUrlNormalizer.normalize(youtubeUrl);
		AnalysisJob job = AnalysisJob.requestedRange(normalized, startSeconds, endSeconds);
		analysisJobRepository.save(job);
		return job;
	}

	@Transactional
	public AnalysisJob createJobAt(String youtubeUrl, double atSeconds, double windowSeconds) {
		String normalized = YoutubeUrlNormalizer.normalize(youtubeUrl);
		AnalysisJob job = AnalysisJob.requestedAt(normalized, atSeconds, windowSeconds);
		analysisJobRepository.save(job);
		return job;
	}
	...
	@Transactional(readOnly = true)
	public Optional<AnalysisJob> findLatestByYoutubeUrlRange(String youtubeUrl, double startSeconds, double endSeconds) {
		String raw = youtubeUrl == null ? null : youtubeUrl.trim();
		if (raw == null || raw.isBlank()) {
			return Optional.empty();
		}
		...
		Optional<AnalysisJob> latest = analysisJobRepository
			.findFirstByYoutubeUrlAndRequestModeAndRangeStartSecondsAndRangeEndSecondsOrderByCreatedAtDesc(
				normalized,
				AnalysisRequestMode.RANGE,
				startSeconds,
				endSeconds
			);
		if (latest.isPresent()) {
	...
	@Transactional(readOnly = true)
	public Optional<AnalysisJob> findLatestByYoutubeUrlAt(String youtubeUrl, double atSeconds, double windowSeconds) {
		String raw = youtubeUrl == null ? null : youtubeUrl.trim();
		if (raw == null || raw.isBlank()) {
			return Optional.empty();
		}
		String normalized = YoutubeUrlNormalizer.normalize(raw);
		Optional<AnalysisJob> latest = analysisJobRepository
			.findFirstByYoutubeUrlAndRequestModeAndAtSecondsAndWindowSecondsOrderByCreatedAtDesc(
				normalized,
				AnalysisRequestMode.AT,
				atSeconds,
				windowSeconds
			);
		if (latest.isPresent()) {
