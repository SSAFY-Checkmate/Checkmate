...
class CleansingResponse(BaseModel):
    """Response model for the Pre-filtering (Cleansing) stage."""
    
    thought_process: str = Field(
        description="Brief thought process analyzing the chunk of sentences to determine which ones contain objective facts"
    )
    
    factual_sentence_indices: List[int] = Field(
        description="List of integer IDs (indices) of the sentences that contain objective, verifiable propositions. Exclude purely subjective opinions, generic statements, and greetings."
    )
